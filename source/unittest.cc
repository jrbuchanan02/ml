...
void inverseTest()
{
    ...
    bool passes = (result * test) == Matrix<double>::identity(3) || (test * result) == Matrix<double>::identity(3);
    std::cout << (passes ? " Yes" : " No") << "\n";

    std::cout << "Product of test and its supposed inverse:\n";
    auto product = result * test;
    std::cout << product[0][0] << " " << product[0][1] << " " << product[0][2] << "\n";
    std::cout << product[1][0] << " " << product[1][1] << " " << product[1][2] << "\n";
    std::cout << product[2][0] << " " << product[2][1] << " " << product[2][2] << "\n";
    std::cout << "Reversed:\n";
    product = test * result;
    std::cout << product[0][0] << " " << product[0][1] << " " << product[0][2] << "\n";
    std::cout << product[1][0] << " " << product[1][1] << " " << product[1][2] << "\n";
...
void echelonTest()
{
    ...
    std::cout << "[" << result[0][0] << "," << result[0][1] << ";" << result[1][0] << "," << result[1][1] << "]\n";
    ...
    std::cout << "Actual:   [";
    std::cout << result[0][0] << "," << result[0][1] << "," << result[0][2] << "," << result[0][3] << ";";
    std::cout << result[1][0] << "," << result[1][1] << "," << result[1][2] << "," << result[1][3] << ";";
    std::cout << result[2][0] << "," << result[2][1] << "," << result[2][2] << "," << result[2][3] << "]\n";
}
...
void testMatrixMultiplication()
{
    ...
    expect[0] = std::vector<long double>{1 * 10 + 2 * 13 + 3 * 16, 1 * 11 + 2 * 14 + 3 * 17, 1 * 12 + 2 * 15 + 3 * 18};
    expect[1] = std::vector<long double>{4 * 10 + 5 * 13 + 6 * 16, 4 * 11 + 5 * 14 + 6 * 17, 4 * 12 + 5 * 15 + 6 * 18};
    expect[2] = std::vector<long double>{7 * 10 + 8 * 13 + 9 * 16, 7 * 11 + 8 * 14 + 9 * 17, 7 * 12 + 8 * 15 + 9 * 18};

    std::cout << "Expected: ";
    for (std::size_t i = 0; i < 3; i++)
    {
        std::cout << expect[i][0] << ", " << expect[i][1] << ", " << expect[i][2] << "; ";
    }
    std::cout << "\n";
    std::cout << "Actual: ";
    for (std::size_t i = 0; i < 3; i++)
